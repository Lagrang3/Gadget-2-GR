...
void pm_init_periodic(void)
{
    ...
    MPI_Allreduce(&nslab_x, &smallest_slab, 1, MPI_INT, MPI_MIN,
                  MPI_COMM_WORLD);
                ...
                if (level > 0)
                {
                    MPI_Sendrecv(
                        ...
                        (sendmax - sendmin + 1) * dimy * dimz *
                            sizeof(fftw_real),
                        MPI_BYTE, recvTask, TAG_PERIODIC_A, forcegrid,
                        (recvmax - recvmin + 1) * recv_dimy * recv_dimz *
                            sizeof(fftw_real),
                        MPI_BYTE, recvTask, TAG_PERIODIC_A, MPI_COMM_WORLD,
                        &status);
                }
                else
                {
                    ...
                    if (slab_xx >= 0 && slab_xx < slabs_per_task[ThisTask])
                    {
                             ...
                             slab_y++)
                        {
                                 ...
                                 slab_z++)
                            {
                                ...
                                rhogrid[PMGRID * PMGRID2 * slab_xx +
                                        PMGRID2 * slab_yy +
                                        slab_zz] += forcegrid
        ...
        for (x = 0; x < PMGRID; x++)
            for (z = 0; z < PMGRID / 2 + 1; z++)
            {
                ...
                if (k2 > 0)
                {
                    ...
                    ip = PMGRID * (PMGRID / 2 + 1) * (y - slabstart_y) +
                         (PMGRID / 2 + 1) * x + z;
...
void pmpotential_periodic(void)
{
                    ...
                    0) /* ok, we have a contribution to the slab */
            {
                ...
                if (level > 0)
                {
                    MPI_Sendrecv(
                        ...
                        (sendmax - sendmin + 1) * dimy * dimz *
                            sizeof(fftw_real),
                        MPI_BYTE, recvTask, TAG_PERIODIC_C, forcegrid,
                        (recvmax - recvmin + 1) * recv_dimy * recv_dimz *
                            sizeof(fftw_real),
                        MPI_BYTE, recvTask, TAG_PERIODIC_C, MPI_COMM_WORLD,
                        &status);
                }
                else
                {
    ...
    for (i = 0; i < NumPart; i++)
    {
        ...
        P[i].Potential += forcegrid[(slab_x * dimy + slab_y) * dimz + slab_z] *
                          (1.0 - dx) * (1.0 - dy) * (1.0 - dz);
        ...
        P[i].Potential += forcegrid[(slab_x * dimy + slab_y) * dimz + slab_zz] *
                          (1.0 - dx) * (1.0 - dy) * dz;
        P[i].Potential +=
            forcegrid[(slab_x * dimy + slab_yy) * dimz + slab_zz] * (1.0 - dx) *
            dy * dz;

        P[i].Potential += forcegrid[(slab_xx * dimy + slab_y) * dimz + slab_z] *
                          (dx) * (1.0 - dy) * (1.0 - dz);
        P[i].Potential +=
            forcegrid[(slab_xx * dimy + slab_yy) * dimz + slab_z] * (dx)*dy *
            (1.0 - dz);
        P[i].Potential +=
            forcegrid[(slab_xx * dimy + slab_y) * dimz + slab_zz] * (dx) *
            (1.0 - dy) * dz;
        P[i].Potential +=
            forcegrid[(slab_xx * dimy + slab_yy) * dimz + slab_zz] * (dx)*dy *
            dz;
    }

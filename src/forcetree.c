...
int force_treebuild_single(int npart)
{
    ...
    for (i = 0; i < npart; i++)
    {
        ...
        while (TopNodes[no].Daughter >= 0)
            no = TopNodes[no].Daughter +
                 (key - TopNodes[no].StartKey) / (TopNodes[no].Size / 8);
        ...
        while (1)
        {
            ...
            else
            {
                ...
                if (nfreep->len < 1.0e-3 * epsilon)
                {
                     ...
                     * extremely close) locations. Randomize subnode index to
                     * allow tree construction. Note: Multipole moments of tree
                     * are still correct, but this will only happen well below
                     * gravitational softening length-scale anyway.
                     */
                ...
                {
                    printf(
                        "task %d: maximum number %d of tree-nodes reached.\n",
                        ThisTask, MaxNodes);
    ...
    if (TopNodes[topnode].Daughter >= 0)
    {
        for (i = 0; i < 2; i++)
            for (j = 0; j < 2; j++)
                for (k = 0; k < 2; k++)
                {
                    sub = 7 & peano_hilbert_key((x << 1) + i, (y << 1) + j,
                                                (z << 1) + k, bits);
                    ...
                    Nodes[*nextfree].center[0] =
                        Nodes[no].center[0] +
                        (2 * i - 1) * 0.25 * Nodes[no].len;
                    Nodes[*nextfree].center[1] =
                        Nodes[no].center[1] +
                        (2 * j - 1) * 0.25 * Nodes[no].len;
                    Nodes[*nextfree].center[2] =
                        Nodes[no].center[2] +
                        (2 * k - 1) * 0.25 * Nodes[no].len;

                    for (n = 0; n < 8; n++)
                        Nodes[*nextfree].u.suns[n] = -1;

                    if (TopNodes[TopNodes[topnode].Daughter + sub].Daughter ==
                        -1)
                        DomainNodeIndex
                    ...
                    *nodecount = *nodecount + 1;
                    ...
                    {
                        printf(
                            "task %d: maximum number %d of tree-nodes "
                            "reached.\n",
                            ThisTask, MaxNodes);
                    ...
                    force_create_empty_nodes(*nextfree - 1,
                                             TopNodes[topnode].Daughter + sub,
                                             bits + 1, 2 * x + i, 2 * y + j,
                                             2 * z + k, nodecount, nextfree);
                }
    }
}
...
void force_insert_pseudo_particles(void)
{
    ...
    for (i = 0; i < NTopleaves; i++)
    {
        if (i < DomainMyStart || i > DomainMyLast)
        {
            ...
            {
                if (th >=
                    All.MaxPart) /* we are dealing with an internal node */
                {
...
#    endif
    ...
    if (no >= All.MaxPart && no < All.MaxPart + MaxNodes) /* internal node */
    {
        for (j = 0; j < 8; j++)
            suns[j] =
                Nodes[no].u.suns[j]; /* this "backup" is necessary because the
...
#    ifndef ADAPTIVE_GRAVSOFT_FORGAS
                        ...
                        else
                        {
                            ...
                            {
                                if (All.ForceSoftening[(
                                        (Nodes[p].u.d.bitflags >> 2) & 7)] >
...
#    endif
    ...
    for (level = 1; level < (1 << PTask); level++)
    {
        ...
        if (recvTask < NTask)
            MPI_Sendrecv(
                &DomainMoment[DomainStartList[sendTask]],
...
#    endif
            ...
            do
            {
                ...
                for (k = 0; k < 3; k++)
                {
                    ...
                    {
                        Nodes[no].u.d.s[k] =
                            (Nodes[no].u.d.mass * Nodes[no].u.d.s[k] +
                             massnew * snew[k] - massold * sold[k]) /
                            mm;
                        Extnodes[no].vs[k] =
                            (Nodes[no].u.d.mass * Extnodes[no].vs[k] +
...
#    ifndef ADAPTIVE_GRAVSOFT_FORGAS
                ...
                Nodes[no].u.d.bitflags = (Nodes[no].u.d.bitflags & 3) +
                                         4 * maxsofttype + 32 * diffsoftflag;
#    else
...
void force_update_len(void)
{
    ...
    for (level = 1; level < (1 << PTask); level++)
    {
        ...
        if (recvTask < NTask)
            MPI_Sendrecv(
                &DomainTreeNodeLen[DomainStartList[sendTask]],
                (DomainEndList[sendTask] - DomainStartList[sendTask] + 1) *
                    sizeof(FLOAT),
...
void force_update_hmax(void)
{
    ...
    for (level = 1; level < (1 << PTask); level++)
    {
        ...
        if (recvTask < NTask)
            MPI_Sendrecv(
                &DomainHmax[DomainStartList[sendTask]],
                (DomainEndList[sendTask] - DomainStartList[sendTask] + 1) *
                    sizeof(FLOAT),
...
#    endif
        ...
        else /* we have an  internal node. Need to check opening criterion */
        {
            if (mode == 1)
            {
                if ((nop->u.d.bitflags & 3) ==
                    1) /* if it's a top-level node
            ...
            {
                if (nop->len * nop->len >
                    r2 * All.ErrTolTheta * All.ErrTolTheta)
                {
                    /* open cell */
                    no = nop->u.d.nextnode;
                    continue;
...
#    ifndef ADAPTIVE_GRAVSOFT_FORGAS
            ...
            if (maxsofttype ==
                7) /* may only occur for zero mass top-level nodes */
            {
                if (mass > 0)
                    endrun(986);
                no = nop->u.d.nextnode;
                ...
                if (h < All.ForceSoftening[maxsofttype])
                {
                    ...
                    if (r2 < h * h)
                    {
                        if (((nop->u.d.bitflags >> 5) &
                             1)) /* bit-5 signals that there are particles of
                                        ...
                                        double aold)
{
    ...
    while (no >= 0)
    {
        ...
        acc_x +=
            mass * signx *
            (fcorrx[i][j][k] * f1 + fcorrx[i][j][k + 1] * f2 +
             fcorrx[i][j + 1][k] * f3 + fcorrx[i][j + 1][k + 1] * f4 +
             fcorrx[i + 1][j][k] * f5 + fcorrx[i + 1][j][k + 1] * f6 +
...
#    endif
        ...
        else /* we have an  internal node. Need to check opening criterion */
        {
            ...
            {
                if ((nop->u.d.bitflags & 3) ==
                    1) /* if it's a top-level node which does not contain local
                          particles */
                {
                    no = nop->u.d.sibling;
                    continue;
            ...
            dyy = nop->center[1] -
                  pos_y; /* this vector is -y in my thesis notation */
...
#    endif
                ...
                if (u < 0.5)
                    wp = -2.8 +
                         u * u * (5.333333333333 + u * u * (6.4 * u - 9.6));
                else
                    wp = -3.2 + 0.066666666667 / u +
                         u * u *
                             (10.666666666667 +
...
void force_treeallocate(int maxnodes, int maxpart)
{
    ...
    if (!(Extnodes_base =
              malloc(bytes = (MaxNodes + 1) * sizeof(struct extNODE))))
    {
...
#    endif
    ...
    {
        epsilon =
            dmax(All.ForceSoftening[P[i].Type], All.ForceSoftening[ptype]);
...
#    endif
        ...
        {
            if (u < 0.5)
                fac = P[i].Mass * h_inv * h_inv * h_inv *
                      (10.666666666667 + u * u * (32.0 * u - 38.4));
            else
                fac = P[i].Mass * h_inv * h_inv * h_inv *
                      (21.333333333333 - 48.0 * u + 38.4 * u * u -
...
#    endif
    ...
    if ((fd = fopen(buf, "r")))
    {
        ...
        my_fread(&fcorrx[0][0][0], sizeof(FLOAT),
                 (EN + 1) * (EN + 1) * (EN + 1), fd);
        my_fread(&fcorry[0][0][0], sizeof(FLOAT),
                 (EN + 1) * (EN + 1) * (EN + 1), fd);
        my_fread(&fcorrz[0][0][0], sizeof(FLOAT),
                 (EN + 1) * (EN + 1) * (EN + 1), fd);
        my_fread(&potcorr[0][0][0], sizeof(FLOAT),
                 (EN + 1) * (EN + 1) * (EN + 1), fd);
        fclose(fd);
    }
    else
...
#    ifdef DOUBLEPRECISION
            MPI_Bcast(&fcorrx[0][0][beg], len, MPI_DOUBLE, task,
                      MPI_COMM_WORLD);
            MPI_Bcast(&fcorry[0][0][beg], len, MPI_DOUBLE, task,
                      MPI_COMM_WORLD);
            MPI_Bcast(&fcorrz[0][0][beg], len, MPI_DOUBLE, task,
                      MPI_COMM_WORLD);
            MPI_Bcast(&potcorr[0][0][beg], len, MPI_DOUBLE, task,
                      MPI_COMM_WORLD);
#    else
            ...
            MPI_Bcast(&potcorr[0][0][beg], len, MPI_FLOAT, task,
                      MPI_COMM_WORLD);
#    endif
        }
        ...
        if (ThisTask == 0)
        {
            ...
            if ((fd = fopen(buf, "w")))
            {
                my_fwrite(&fcorrx[0][0][0], sizeof(FLOAT),
                          (EN + 1) * (EN + 1) * (EN + 1), fd);
                my_fwrite(&fcorry[0][0][0], sizeof(FLOAT),
                          (EN + 1) * (EN + 1) * (EN + 1), fd);
                my_fwrite(&fcorrz[0][0][0], sizeof(FLOAT),
                          (EN + 1) * (EN + 1) * (EN + 1), fd);
                my_fwrite(&potcorr[0][0][0], sizeof(FLOAT),
                          (EN + 1) * (EN + 1) * (EN + 1), fd);
                fclose(fd);
            }
        }
...
double ewald_pot_corr(double dx, double dy, double dz)
{
    ...
    return potcorr[i][j][k] * f1 + potcorr[i][j][k + 1] * f2 +
           potcorr[i][j + 1][k] * f3 + potcorr[i][j + 1][k + 1] * f4 +
           potcorr[i + 1][j][k] * f5 + potcorr[i + 1][j][k + 1] * f6 +
           potcorr[i + 1][j + 1][k] * f7 + potcorr[i + 1][j + 1][k + 1] * f8;
}

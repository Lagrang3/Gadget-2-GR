...
void run(void)
{
    ...
    do /* main loop */
    {
        ...
        find_next_sync_point_and_drift(); /* find next synchronization point and
                                           * drift particles to this time. If
                                           * needed, this function will also
                                           * write an output file at the desired
                                           * time.
                                           */
        ...
        if (ThisTask == 0)
        {
            if ((CPUThisRun - All.TimeLastRestartFile) >=
...
void find_next_sync_point_and_drift(void)
{
    ...
    /* Determine 'NumForceUpdate', i.e. the number of particles on this
     * processor that are going to be active */
    ...
    MPI_Allgather(&NumForceUpdate, 1, MPI_INT, temp, 1, MPI_INT,
                  MPI_COMM_WORLD);
    for (n = 0; n < NTask; n++)
    ...
    while (min_glob >= All.Ti_nextoutput && All.Ti_nextoutput >= 0)
    {
        ...
        if (All.ComovingIntegrationOn)
            All.Time =
                All.TimeBegin * exp(All.Ti_Current * All.Timebase_interval);
        else
...
void every_timestep_stuff(void)
{
    ...
    if (ThisTask == 0)
    {
        ...
        fprintf(
            ...
            "%10.2f %10.2f %10.2f %10.2f %10.2f %10.2f %10.2f %10.2f %10.2f\n",
            All.CPU_Total, All.CPU_Gravity, All.CPU_Hydro, All.CPU_Domain,
            All.CPU_Potential, All.CPU_Predict, All.CPU_TimeLine,
            All.CPU_Snapshot, All.CPU_TreeWalk, All.CPU_TreeConstruction,
            All.CPU_CommSum, All.CPU_Imbalance, All.CPU_HydCompWalk,
            All.CPU_HydCommSumm, All.CPU_HydImbalance, All.CPU_EnsureNgb,
            All.CPU_PM, All.CPU_Peano);
        fflush(FdCPU);
    }
...
void energy_statistics(void)
{
    ...
    if (ThisTask == 0)
    {
        fprintf(FdEnergy,
                ...
                "%g %g %g %g %g %g %g %g\n",
                All.Time, SysState.EnergyInt, SysState.EnergyPot,
                SysState.EnergyKin, SysState.EnergyIntComp[0],
                SysState.EnergyPotComp[0], SysState.EnergyKinComp[0],
                SysState.EnergyIntComp[1], SysState.EnergyPotComp[1],
                SysState.EnergyKinComp[1], SysState.EnergyIntComp[2],
                SysState.EnergyPotComp[2], SysState.EnergyKinComp[2],
                SysState.EnergyIntComp[3], SysState.EnergyPotComp[3],
                SysState.EnergyKinComp[3], SysState.EnergyIntComp[4],
                SysState.EnergyPotComp[4], SysState.EnergyKinComp[4],
                SysState.EnergyIntComp[5], SysState.EnergyPotComp[5],
                SysState.EnergyKinComp[5], SysState.MassComp[0],
                SysState.MassComp[1], SysState.MassComp[2],
                SysState.MassComp[3], SysState.MassComp[4],
                SysState.MassComp[5]);

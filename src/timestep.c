...
/*! \file timestep.c
 *  \brief routines for 'kicking' particles in momentum space and assigning new
 * timesteps
 */
...
void advance_and_find_timesteps(void)
{
    ...
    if (All.ComovingIntegrationOn)
        ...
        dt_gravkickB =
            (All.Ti_Current - (All.PM_Ti_begstep + All.PM_Ti_endstep) / 2) *
    ...
    dmean = pow(P[0].Mass / (All.Omega0 * 3 * All.Hubble * All.Hubble /
                             (8 * M_PI * All.G)),
                1.0 / 3);
    ...
    for (i = 0, dispmax = 0; i < NumPart; i++)
    {
        for (j = 0; j < 3; j++)
        {
            P[i].Vel[j] = 0;
            P[i].Pos[j] +=
                fac * P[i].GravAccel[j] * 2.0 / (3 * All.Hubble * All.Hubble);
            P[i].GravAccel[j] = 0;
        }
    }
#endif
    ...
    if (ThisTask == 0)
        printf("Syn Range = %g  PresentMinStep = %d  PresentMaxStep = %d \n",
               (double)All.PresentMaxStep / All.PresentMinStep,
               All.PresentMinStep, All.PresentMaxStep);
    ...
    for (i = 0; i < NumPart; i++)
    {
        if (P[i].Ti_endstep == All.Ti_Current)
        {
            ...
            ti_step =
                ((P[i].Ti_endstep + ti_grp + ti_step) / ti_step) * ti_step -
...
#    ifdef PSEUDOSYMMETRIC
            if (P[i].Type != 0)
            {
                ...
                {
                    apred = aphys + ((aphys - P[i].AphysOld) /
                    ...
                    if (fabs(apred - aphys) < 0.5 * aphys)
                    {
                        ...
                        if (ti_step2 < ti_step)
                        {
                            ...
                            prob = ((apred - aphys) / (aphys - P[i].AphysOld) *
                                    (P[i].Ti_endstep - P[i].Ti_begstep)) /
                                   ti_step;
...
#    ifdef SYNCHRONIZATION
            if (ti_step > (P[i].Ti_endstep -
                           P[i].Ti_begstep)) /* timestep wants to increase */
            {
                if (((TIMEBASE - P[i].Ti_endstep) % ti_step) > 0)
                    ti_step = P[i].Ti_endstep -
                              P[i].Ti_begstep; /* leave at old step */
            }
#    endif
#endif /* end of FLEXSTEPS */

            if (All.Ti_Current ==
                TIMEBASE) /* we here finish the last timestep. */
                ti_step = 0;

            if ((TIMEBASE - All.Ti_Current) <
                ti_step) /* check that we don't run beyond the end */
                ti_step = TIMEBASE - All.Ti_Current;

            tstart = (P[i].Ti_begstep + P[i].Ti_endstep) /
                     2;                           /* midpoint of old step */
            ...
            else
            {
                dt_entr = dt_gravkick = dt_hydrokick =
                    (tend - tstart) * All.Timebase_interval;
            ...
            if (P[i].Type == 0) /* SPH stuff */
            {
                ...
                if (All.MinEgySpec)
                {
                    minentropy = All.MinEgySpec * GAMMA_MINUS1 /
                ...
                if (SphP[i].Entropy + SphP[i].DtEntropy * dt_entr <
                    0.5 * SphP[i].Entropy)
             ...
             * dynamically.
             */
            if (All.NumForcesSinceLastDomainDecomp <
                All.TotNumPart * All.TreeDomainUpdateFrequency)
            {
                no = Father[i];
                while (no >= 0)
                {
                    for (j = 0; j < 3; j++)
                        Extnodes[no].vs[j] +=
                            dv[j] * P[i].Mass / Nodes[no].u.d.mass;
    ...
    if (All.PM_Ti_endstep == All.Ti_Current) /* need to do long-range kick */
    {
        ...
        if (ti_step > (All.PM_Ti_endstep -
                       All.PM_Ti_begstep)) /* PM-timestep wants to increase */
        {
            /* we only increase if an integer number of steps will bring us to
             * the end */
            if (((TIMEBASE - All.PM_Ti_endstep) % ti_step) > 0)
                ti_step = All.PM_Ti_endstep -
                          All.PM_Ti_begstep; /* leave at old step */
        }
        ...
        if (All.ComovingIntegrationOn)
            dt_gravkickB = -get_gravkick_factor(
                All.PM_Ti_begstep, (All.PM_Ti_begstep + All.PM_Ti_endstep) / 2);
        else
            dt_gravkickB = -((All.PM_Ti_begstep + All.PM_Ti_endstep) / 2 -
                             All.PM_Ti_begstep) *
                           All.Timebase_interval;
        ...
        for (i = 0; i < NumPart; i++)
        {
            ...
            if (P[i].Type == 0)
            {
                ...
                {
                    dt_gravkickA =
                        get_gravkick_factor(P[i].Ti_begstep, All.Ti_Current) -
                        get_gravkick_factor(
                            P[i].Ti_begstep,
                            (P[i].Ti_begstep + P[i].Ti_endstep) / 2);
                    dt_hydrokick =
                        get_hydrokick_factor(P[i].Ti_begstep, All.Ti_Current) -
				   ...
				   aphys */ )
{
    ...
    if (flag == 0)
    {
        ...
        ac = sqrt(ax * ax + ay * ay +
                  az * az); /* this is now the physical acceleration */
        *aphys = ac;
    }
    else
        ac = *aphys;
    ...
    switch (All.TypeOfTimestepCriterion)
    {
        case 0:
            if (flag > 0)
            {
                ...
                ac = 2 * All.ErrTolIntAccuracy * atime *
                     All.SofteningTable[P[p].Type] / (dt * dt);
                *aphys = ac;
                return flag;
            }
    ...
    if (P[p].Type == 0)
    {
        ...
        if (All.ComovingIntegrationOn)
            dt_courant = 2 * All.CourantFac * All.Time * SphP[p].Hsml /
                         (fac3 * SphP[p].MaxSignalVel);
        else
            dt_courant =
    ...
    /* convert the physical timestep to dloga if needed. Note: If comoving
       integration has not been selected, hubble_a=1.
     */
    dt *= hubble_a;
    ...
    if (dt < All.MinSizeTimestep)
    {
        ...
        printf(
            "warning: Timestep wants to be below the limit "
            "`MinSizeTimestep'\n");
        ...
        if (P[p].Type == 0)
        {
            printf(
                ...
                (int)P[p].ID, dt, dt_courant * hubble_a, ac, P[p].Pos[0],
                P[p].Pos[1], P[p].Pos[2], SphP[p].Hsml, SphP[p].MaxSignalVel,
                sqrt(2 * All.ErrTolIntAccuracy * atime *
                     All.SofteningTable[P[p].Type] / ac) *
                    hubble_a,
        ...
        else
        {
            printf("Part-ID=%d  dt=%g ac=%g xyz=(%g|%g|%g)\n", (int)P[p].ID, dt,
                   ac, P[p].Pos[0], P[p].Pos[1], P[p].Pos[2]);
        }
        fflush(stdout);
        endrun(888);
#endif
...
void find_dt_displacement_constraint(double hfac /*!<  should be  a^2*H(a)  */)
{
    ...
    if (All.ComovingIntegrationOn)
    {
        ...
        for (i = 0; i < NumPart; i++)
        {
            v[P[i].Type] += P[i].Vel[0] * P[i].Vel[0] +
                            P[i].Vel[1] * P[i].Vel[1] +
                            P[i].Vel[2] * P[i].Vel[2];
        ...
        for (type = 0; type < 6; type++)
        {
            if (count_sum[type] > 0)
            {
                if (type == 0)
                    dmean =
                        pow(min_mass[type] / (All.OmegaBaryon * 3 * All.Hubble *
                                              All.Hubble / (8 * M_PI * All.G)),
                            1.0 / 3);
                else
                    dmean =
                        pow(min_mass[type] /
                                ((All.Omega0 - All.OmegaBaryon) * 3 *
                                 All.Hubble * All.Hubble / (8 * M_PI * All.G)),
                            1.0 / 3);

                dt = All.MaxRMSDisplacementFac * hfac * dmean /
                     sqrt(v_sum[type] / count_sum[type]);
...
#    endif
                if (asmth < dmean)
                    dt = All.MaxRMSDisplacementFac * hfac * asmth /
                         sqrt(v_sum[type] / count_sum[type]);
#endif

                if (ThisTask == 0)
                    printf(
                        "type=%d  dmean=%g asmth=%g minmass=%g a=%g  "

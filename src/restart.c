...
void restart(int modus)
{
    ...
    {
        printf(
            "Fatal error.\nNumber of processors must be a smaller or equal "
            "than `NumFilesWrittenInParallel'.\n");
        endrun(2131);
    }
    ...
    {
        if (ThisTask ==
            (masterTask + groupTask)) /* ok, it's this processor's turn */
        {
            if (modus)
            {
                if (!(fd = fopen(buf, "r")))
            ...
            if (modus) /* read */
            {
                if (All.PartAllocFactor != save_PartAllocFactor)
                {
                    ...
                    All.MaxPart =
                        All.PartAllocFactor * (All.TotNumPart / NTask);
                    All.MaxPartSph =
                        All.PartAllocFactor * (All.TotN_gas / NTask);
                ...
                {
                    printf(
                        "The restart file on task=%d is not consistent with "
                        "the one on task=0\n",
                        ThisTask);
                    fflush(stdout);
                    endrun(16);
            ...
            if (N_gas > 0)
            {
                ...
                {
                    printf(
                        "SPH: it seems you have reduced(!) 'PartAllocFactor' "
                        "below the value of %g needed to load the restart "
                        "file.\n",
                ...
                /* Sph-Particle data  */
                byten(&SphP[0], N_gas * sizeof(struct sph_particle_data),
                      modus);
            }

            /* write state of random number generator */
            byten(gsl_rng_state(random_generator),
                  gsl_rng_size(random_generator), modus);
            ...
            if (modus) /* read */
            {
                ...
                force_treeallocate(All.TreeAllocFactor * All.MaxPart,
                                   All.MaxPart);
            }
            ...
            {
                printf(
                    "Tree storage: it seems you have reduced(!) "
                    "'PartAllocFactor' below the value needed to load the "
                    "restart file (task=%d). "
            ...
            if (modus) /* read */
                    ...
                    All.TreeAllocFactor != save_TreeAllocFactor)
                {
                    ...
                    for (i = 0; i < Numnodestree; i++)
                    {
                        ...
                        {
                            if (Nodes_base[i].u.d.sibling >=
                                old_MaxPart + old_MaxNodes)
                                Nodes_base[i].u.d.sibling +=
                                    (All.MaxPart - old_MaxPart) +

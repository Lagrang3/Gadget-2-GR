...
#    endif
    ...
    if (All.ComovingIntegrationOn)
    {
        ...
        hubble_a = hubble_a2 = atime = fac_mu = fac_vsic_fix = a3inv = fac_egy =
            1.0;

    /* `NumSphUpdate' gives the number of particles on this processor that want
     * a force update */
    ...
    MPI_Allgather(&NumSphUpdate, 1, MPI_INT, numlist, 1, MPI_INT,
                  MPI_COMM_WORLD);
    ...
    noffset =
        malloc(sizeof(int) * NTask); /* offsets of bunches in common list */
    ...
    while (ntotleft > 0)
    {
        ...
        for (nexport = 0, ndone = 0;
             i < N_gas && nexport < All.BunchSizeHydro - NTask; i++)
            ...
            for (ngrp = level; ngrp < (1 << PTask); ngrp++)
            {
                ...
                if (recvTask < NTask)
                {
                        ...
                        nsend[recvTask * NTask + ThisTask] > 0)
                    {
                        /* get the particles */
                        MPI_Sendrecv(
                            &HydroDataIn[noffset[recvTask]],
                            nsend_local[recvTask] * sizeof(struct hydrodata_in),
                            MPI_BYTE, recvTask, TAG_HYDRO_A,
                            &HydroDataGet[nbuffer[ThisTask]],
                            nsend[recvTask * NTask + ThisTask] *
                                sizeof(struct hydrodata_in),
                            MPI_BYTE, recvTask, TAG_HYDRO_A, MPI_COMM_WORLD,
                            &status);
                    }
                }
                ...
                if (recvTask < NTask)
                {
                        ...
                        nsend[recvTask * NTask + ThisTask] > 0)
                    {
                        ...
                        for (j = 0; j < nsend_local[recvTask]; j++)
                        {
                            ...
                            for (k = 0; k < 3; k++)
                                SphP[place].HydroAccel[k] +=
                                    HydroDataPartialResult[source].Acc[k];

                            SphP[place].DtEntropy +=
                                HydroDataPartialResult[source].DtEntropy;
    ...
    for (i = 0; i < N_gas; i++)
        ...
        {
            SphP[i].DtEntropy *=
                GAMMA_MINUS1 / (hubble_a2 * pow(SphP[i].Density, GAMMA_MINUS1));
    ...
    MPI_Reduce(&timecommsumm, &sumcomm, 1, MPI_DOUBLE, MPI_SUM, 0,
               MPI_COMM_WORLD);
    ...
    do
    {
        ...
        for (n = 0; n < numngb; n++)
        {
            ...
            if (r2 < h_i2 || r2 < h_j * h_j)
            {
                ...
                {
                    p_over_rho2_j =
                        SphP[j].Pressure / (SphP[j].Density * SphP[j].Density);
                    soundspeed_j =
                        sqrt(GAMMA * p_over_rho2_j * SphP[j].Density);
                    ...
                    if (r2 < h_i2)
                    {
                        ...
                        if (u < 0.5)
                            dwk_i = hinv4 * u *
                                    (KERNEL_COEFF_3 * u - KERNEL_COEFF_4);
                        else
                            dwk_i =
                                hinv4 * KERNEL_COEFF_6 * (1.0 - u) * (1.0 - u);
                    }
                    else
                    {
                        dwk_i = 0;
                    ...
                    {
                        mu_ij =
                            fac_mu * vdotr2 / r; /* note: this is negative! */
                        ...
                        f2 = fabs(SphP[j].DivVel) /
                             (fabs(SphP[j].DivVel) + SphP[j].CurlVel +
                    ...
                    hfc = hfc_visc +
                          P[j].Mass *
                              (p_over_rho2_i * dwk_i + p_over_rho2_j * dwk_j) /
                              r;

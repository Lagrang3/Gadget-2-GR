...
#define NGB_PERIODIC_X(x)                                                      \
    (xtmp = (x), (xtmp > boxHalf_X)                                            \
                     ? (xtmp - boxSize_X)                                      \
                     : ((xtmp < -boxHalf_X) ? (xtmp + boxSize_X) : xtmp))
#define NGB_PERIODIC_Y(x)                                                      \
    (xtmp = (x), (xtmp > boxHalf_Y)                                            \
                     ? (xtmp - boxSize_Y)                                      \
                     : ((xtmp < -boxHalf_Y) ? (xtmp + boxSize_Y) : xtmp))
#define NGB_PERIODIC_Z(x)                                                      \
    (xtmp = (x), (xtmp > boxHalf_Z)                                            \
                     ? (xtmp - boxSize_Z)                                      \
                     : ((xtmp < -boxHalf_Z) ? (xtmp + boxSize_Z) : xtmp))
    ...
    while (no >= 0)
    {
        if (no < All.MaxPart) /* single particle */
        {
            ...
            if (numngb == MAX_NGB)
            {
                ...
                {
                    printf(
                        "ThisTask=%d: Need to do a second neighbour loop for "
                        "(%g|%g|%g) hsml=%g no=%d\n",

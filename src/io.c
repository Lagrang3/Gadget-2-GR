...
void savepositions(int num)
{
    ...
    All.NumForcesSinceLastDomainDecomp =
        1 + All.TreeDomainUpdateFrequency * All.TotNumPart;
#endif
    ...
    if (NTask < All.NumFilesPerSnapshot)
    {
        if (ThisTask == 0)
            printf(
                "Fatal error.\nNumber of processors must be larger or equal "
                "than All.NumFilesPerSnapshot.\n");
        endrun(0);
    }
                ...
                if (P[pindex].Type == type)
                {
                    if (All.ComovingIntegrationOn)
                    {
                        dt_gravkick =
                            ...
                            get_gravkick_factor(
                                P[pindex].Ti_begstep,
                                (P[pindex].Ti_begstep + P[pindex].Ti_endstep) /
                                    2);
                        dt_hydrokick =
                    ...
                    {
                        fp[k] = P[pindex].Vel[k] +
                                P[pindex].GravAccel[k] * dt_gravkick;
                        if (P[pindex].Type == 0)
...
#    endif
        ...
        case IO_TSTP: /* timestep  */
            ...
            for (n = 0; n < pc; pindex++)
                if (P[pindex].Type == type)
                {
                    *fp++ = (P[pindex].Ti_endstep - P[pindex].Ti_begstep) *
                            All.Timebase_interval;
                    n++;
                }
#endif
        ...
        if (blockpresent(blocknr))
        {
            ...
            if (npart > 0)
            {
                if (ThisTask == writeTask)
                {
                    if (All.SnapFormat == 1 || All.SnapFormat == 2)
                    {
                        if (All.SnapFormat == 2)
                        {
                            blksize = sizeof(int) + 4 * sizeof(char);
                            SKIP;
                            my_fwrite(Tab_IO_Labels[blocknr], sizeof(char), 4,
                                      fd);
                            nextblock = npart * bytes_per_blockelement +
                                        2 * sizeof(int);
                            my_fwrite(&nextblock, sizeof(int), 1, fd);
                            SKIP;
                        }
                ...
                for (type = 0; type < 6; type++)
                {
                    ...
                    {
#ifdef HAVE_HDF5
                        if (ThisTask == writeTask && All.SnapFormat == 3 &&
                            header.npart[type] > 0)
                        {
                        ...
                        for (task = writeTask, offset = 0; task <= lastTask;
                             task++)
                        {
                            if (task == ThisTask)
                            {
                            ...
                            while (n_for_this_task > 0)
                            {
                                ...
                                if (ThisTask == task)
                                    fill_write_buffer(blocknr, &offset, pc,
                                                      type);

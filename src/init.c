...
void init(void)
{
    ...
    if (All.ComovingIntegrationOn)
    {
        All.Timebase_interval =
            (log(All.TimeMax) - log(All.TimeBegin)) / TIMEBASE;
    ...
    if (RestartFlag == 2)
        All.SnapshotFileCount =
            atoi(All.InitCondFile + strlen(All.InitCondFile) - 3) + 1;
     ...
     * internal energy, read in from the initial conditions file, unless the
     * file explicitly signals that the initial conditions contain the entropy
     * directly. Once the density has been computed, we can convert thermal
     * energy to entropy.
     */
#ifndef ISOTHERM_EQS
...
void check_omega(void)
{
    ...
    omega = masstot / (All.BoxSize * All.BoxSize * All.BoxSize) /
    ...
    if (fabs(omega - All.Omega0) > 1.0e-3)
    {
        if (ThisTask == 0)
        {
            ...
            printf(
                ...
                "specified Omega=%g in the parameterfile.\n",
                omega, All.Omega0);
            printf("\nI better stop.\n");
...
#ifndef TWODIMS
            SphP[i].Hsml = pow(3.0 / (4 * M_PI) * All.DesNumNgb * P[i].Mass /
                                   Nodes[no].u.d.mass,
                               1.0 / 3) *
                           Nodes[no].len;
#else

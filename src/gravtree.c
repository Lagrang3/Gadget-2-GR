                ...
                if (recvTask < NTask)
                {
                        ...
                        nsend[recvTask * NTask + ThisTask] > 0)
                    {
                        ...
                        for (j = 0; j < nsend_local[recvTask]; j++)
                        {
                            ...
                            P[place].GravCost +=
                                GravDataOut[j + noffset[recvTask]]
                                    .w.Ninteractions;
                        }
                    }
                }
    ...
    for (i = 0; i < NumPart; i++)
        if (P[i].Ti_endstep == All.Ti_Current)
            for (j = 0; j < 3; j++)
                ...
                /* Finally, the following factor allows a computation of a
                   cosmological simulation with vacuum energy in physical
                   coordinates */
#    ifndef PERIODIC
#        ifndef PMGRID
...
#    endif
    ...
    if (ThisTask == 0)
    {
        ...
        fprintf(FdTimings,
                "Nf= %d%09d  total-Nf= %d%09d  ex-frac= %g  iter= %d\n",
        ...
        for (i = 0, maxt = timetreelist[0], sumt = 0, plb_max = 0,
            maxnumnodes = 0, costtotal = 0, sumcomm = 0, ewaldtot = 0;
             i < NTask; i++)
        {

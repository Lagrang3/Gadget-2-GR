...
void begrun(void)
{
    ...
    read_parameter_file(
        ParameterFile); /* ... read in parameters for this run */
...
void set_units(void)
{
    ...
    All.UnitPressure_in_cgs =
        All.UnitMass_in_g / All.UnitLength_in_cm / pow(All.UnitTime_in_s, 2);
    ...
    meanweight =
        4.0 /
        (1 + 3 * HYDROGEN_MASSFRAC); /* note: we assume neutral gas here */

#ifdef ISOTHERM_EQS
    All.MinEgySpec = 0;
#else
    All.MinEgySpec = 1 / meanweight * (1.0 / GAMMA_MINUS1) *
                     (BOLTZMANN / PROTONMASS) * All.MinGasTemp;
...
#define MAXTAGS 300
    ...
    if (ThisTask == 0) /* read parameter file on process 0 */
    {
            ...
            else
            {
                while (!feof(fd))
                {
                    *buf = 0;
                    ...
                    {
                        fprintf(stdout,
                                "Error in file %s:   Tag '%s' not allowed or "
                                "multiple defined.\n",
                                fname, buf1);
                        errorFlag = 1;
                    }
        ...
        if (errorFlag != 2)
            for (i = 0; i < nt; i++)
            {
                ...
                {
                    printf(
                        "Error. I miss a value for tag '%s' in parameter file "
                        "'%s'.\n",
                        tag[i], fname);
                    errorFlag = 1;
                }
    ...
    if (All.NumFilesWrittenInParallel > NTask)
    {
        if (ThisTask == 0)
            printf(
                "NumFilesWrittenInParallel MUST be smaller than number of "
                "processors\n");
        endrun(0);
    }
    ...
    if (All.PeriodicBoundariesOn == 0)
    {
        ...
        {
            printf(
                "Code was compiled with periodic boundary conditions switched "
                "on.\n");
            printf(
                "You must set `PeriodicBoundariesOn=1', or recompile the "
                "code.\n");
        }
        endrun(0);
    }
    ...
    if (All.PeriodicBoundariesOn == 1)
    {
        if (ThisTask == 0)
        {
            printf(
                ...
                "off.\n");
            printf(
                "You must set `PeriodicBoundariesOn=0', or recompile the "
                "code.\n");
        }
        endrun(0);
    }
...
int read_outputlist(char* fname)
{
    ...
    do
    {
        if (fscanf(fd, " %lg ", &All.OutputListTimes[All.OutputListLength]) ==
            1)

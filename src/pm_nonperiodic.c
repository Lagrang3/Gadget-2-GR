...
#        endif
    ...
    for (j = 0; j < 2; j++)
    {
        ...
        All.TotalMeshSize[j] =
            dmax(All.TotalMeshSize[j], All.Xmaxtot[j][1] - All.Xmintot[j][1]);
        All.TotalMeshSize[j] =
            dmax(All.TotalMeshSize[j], All.Xmaxtot[j][2] - All.Xmintot[j][2]);
#        ifdef ENLARGEREGION
...
#        endif
        ...
        for (i = 0; i < 3; i++)
        {
            All.Xmintot[j][i] = (All.Xmintot[j][i] + All.Xmaxtot[j][i]) / 2 -
                                All.TotalMeshSize[j] / 2;
    ...
    /* move lower left corner by two cells to allow finite differencing of the
     * potential by a 4-point function */

    for (j = 0; j < 2; j++)
        for (i = 0; i < 3; i++)
        {
            All.Corner[j][i] =
                All.Xmintot[j][i] - 2.0005 * All.TotalMeshSize[j] / GRID;
            All.UpperCorner[j][i] =
                All.Corner[j][i] +
                (GRID / 2 - 1) * (All.TotalMeshSize[j] / GRID);
        }
...
#        ifdef PLACEHIGHRESREGION
    ...
    {
        All.TotalMeshSize[1] = 2 * (meshinner[1] + 2 * All.Rcut[0]) * (GRID) /
                               ((double)(GRID - 2));
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            All.UpperCorner[1][i] =
                All.Corner[1][i] +
                (GRID / 2 - 1) * (All.TotalMeshSize[1] / GRID);
        }

        if (2 * All.TotalMeshSize[1] / GRID > All.Rcut[0])
        {
            All.TotalMeshSize[1] = 2 * (meshinner[1] + 2 * All.Rcut[0]) *
                                   (GRID) / ((double)(GRID - 10));
            ...
            for (i = 0; i < 3; i++)
            {
                All.Corner[1][i] =
                    All.Xmintot[1][i] -
                    1.0001 * (All.Rcut[0] + 2 * All.TotalMeshSize[1] / GRID);
                All.UpperCorner[1][i] =
                    All.Corner[1][i] +
                    (GRID / 2 - 1) * (All.TotalMeshSize[1] / GRID);
            }
...
#        ifndef PERIODIC
        ...
        printf(
            "(%g|%g|%g)  -> (%g|%g|%g)   ext=%g  totmeshsize=%g  "
            "meshsize=%g\n\n",
            All.Xmintot[0][0], All.Xmintot[0][1], All.Xmintot[0][2],
            All.Xmaxtot[0][0], All.Xmaxtot[0][1], All.Xmaxtot[0][2],
            meshinner[0], All.TotalMeshSize[0], All.TotalMeshSize[0] / GRID);
#        endif
#        ifdef PLACEHIGHRESREGION
...
void pm_init_nonperiodic(void)
{
    ...
    MPI_Allreduce(slab_to_task_local, slab_to_task, GRID, MPI_INT, MPI_SUM,
                  MPI_COMM_WORLD);
    ...
    for (i = slabstart_x; i < (slabstart_x + nslab_x); i++)
        for (j = 0; j < GRID; j++)
            for (k = 0; k < GRID; k++)
            {
                ...
                if (r > 0)
                    kernel[0][GRID * GRID2 * (i - slabstart_x) + GRID2 * j +
                              k] = -fac / r;
                else
                    kernel[0][GRID * GRID2 * (i - slabstart_x) + GRID2 * j +
                              k] = -1 / (sqrt(M_PI) * (((double)ASMTH) / GRID));
            }

    /* do the forward transform of the kernel */

    rfftwnd_mpi(fft_forward_plan, 1, kernel[0], workspace,
...
#        if defined(PLACEHIGHRESREGION)
    ...
    for (i = slabstart_x; i < (slabstart_x + nslab_x); i++)
        for (j = 0; j < GRID; j++)
            for (k = 0; k < GRID; k++)
            {
                ...
                if (r > 0)
                    kernel[1][GRID * GRID2 * (i - slabstart_x) + GRID2 * j +
                              k] = -fac / r;
                else
                {
                    fac = 1 - All.Asmth[1] / All.Asmth[0];
                    kernel[1]
                          [GRID * GRID2 * (i - slabstart_x) + GRID2 * j + k] =
    ...
    rfftwnd_mpi(fft_forward_plan, 1, kernel[1], workspace,
                FFTW_TRANSPOSED_ORDER);
#        endif
        ...
        for (x = 0; x < GRID; x++)
            for (z = 0; z < GRID / 2 + 1; z++)
            {
                ...
                if (k2 > 0)
                {
                    ...
                    ip = GRID * (GRID / 2 + 1) * (y - slabstart_y) +
                         (GRID / 2 + 1) * x + z;
#        if !defined(PERIODIC)
...
int pmforce_nonperiodic(int grnr)
{
    ...
    fac = All.G / pow(All.TotalMeshSize[grnr], 4) *
          pow(All.TotalMeshSize[grnr] / GRID, 3); /* to get potential */
    fac *=
        1 / (2 * All.TotalMeshSize[grnr] / GRID); /* for finite differencing */
...
#        ifdef PLACEHIGHRESREGION
        if (grnr == 0 ||
            (grnr == 1 && ((1 << P[i].Type) & (PLACEHIGHRESREGION))))
#        endif
        {
            for (j = 0; j < 3; j++)
            {
                if (P[i].Pos[j] < All.Xmintot[grnr][j] ||
                    P[i].Pos[j] > All.Xmaxtot[grnr][j])
                {
                    if (flag == 0)
                    {
                        printf(
                            "Particle Id=%d on task=%d with coordinates "
                            "(%g|%g|%g) lies outside PM mesh.\nStopping\n",
                            (int)P[i].ID, ThisTask, P[i].Pos[0], P[i].Pos[1],
                            P[i].Pos[2]);
                        fflush(stdout);
                    }
                    flag++;
        ...
        if (P[i].Pos[0] >= All.Corner[grnr][0] &&
            P[i].Pos[0] < All.UpperCorner[grnr][0])
            if (P[i].Pos[1] >= All.Corner[grnr][1] &&
                P[i].Pos[1] < All.UpperCorner[grnr][1])
                if (P[i].Pos[2] >= All.Corner[grnr][2] &&
                    P[i].Pos[2] < All.UpperCorner[grnr][2])
                {
                    for (j = 0; j < 3; j++)
                    {
                        slab =
                            to_slab_fac * (P[i].Pos[j] - All.Corner[grnr][j]);
    ...
    if (flagsum > 0)
    {
        ...
        return 1; /* error - need to return because particle were outside
                     allowed range */
    }

    MPI_Allgather(meshmin, 3, MPI_INT, meshmin_list, 3, MPI_INT,
                  MPI_COMM_WORLD);
    MPI_Allgather(meshmax, 3, MPI_INT, meshmax_list, 3, MPI_INT,
                  MPI_COMM_WORLD);
    ...
    for (level = 0; level < (1 << PTask);
         level++) /* note: for level=0, target is the same task */
    {
        sendTask = ThisTask;
        ...
        if (recvTask < NTask)
        {
            ...
            recvmin = 2 * GRID;
            recvmax = -1;
            for (slab_x = meshmin_list[3 * recvTask];
                 slab_x < meshmax_list[3 * recvTask] + 2; slab_x++)
            ...
            if ((recvmax - recvmin) >= 0 ||
                (sendmax - sendmin) >=
                    0) /* ok, we have a contribution to the slab */
            {
                recv_dimx = meshmax_list[3 * recvTask + 0] -
                            meshmin_list[3 * recvTask + 0] + 2;
                recv_dimy = meshmax_list[3 * recvTask + 1] -
                            meshmin_list[3 * recvTask + 1] + 2;
                recv_dimz = meshmax_list[3 * recvTask + 2] -
                            meshmin_list[3 * recvTask + 2] + 2;

                if (level > 0)
                {
                    MPI_Sendrecv(
                        workspace + (sendmin - meshmin[0]) * dimy * dimz,
                        (sendmax - sendmin + 1) * dimy * dimz *
                            sizeof(fftw_real),
                        MPI_BYTE, recvTask, TAG_NONPERIOD_A, forcegrid,
                        (recvmax - recvmin + 1) * recv_dimy * recv_dimz *
                            sizeof(fftw_real),
                        MPI_BYTE, recvTask, TAG_NONPERIOD_A, MPI_COMM_WORLD,
                        &status);
                }
                else
                {
                ...
                for (slab_x = recvmin; slab_x <= recvmax; slab_x++)
                {
                    ...
                    if (slab_xx >= 0 && slab_xx < slabs_per_task[ThisTask])
                    {
                        for (slab_y = meshmin_list[3 * recvTask + 1];
                             slab_y <= meshmax_list[3 * recvTask + 1] + 1;
                             slab_y++)
                        {
                            slab_yy = slab_y;
                                 ...
                                 slab_z++)
                            {
                                ...
                                rhogrid[GRID * GRID2 * slab_xx +
                                        GRID2 * slab_yy + slab_zz] += forcegrid
                                    [((slab_x - recvmin) * recv_dimy +
                                      (slab_y -
                                       meshmin_list[3 * recvTask + 1])) *
                                         recv_dimz +
    ...
    for (y = 0; y < nslab_y; y++)
        for (x = 0; x < GRID; x++)
            for (z = 0; z < GRID / 2 + 1; z++)
            {
                ...
                re = fft_of_rhogrid[ip].re * fft_of_kernel[grnr][ip].re -
                     fft_of_rhogrid[ip].im * fft_of_kernel[grnr][ip].im;

                im = fft_of_rhogrid[ip].re * fft_of_kernel[grnr][ip].im +
    ...
    /* if we have a high-res mesh, establish the extension of the local patch in
     * GRID (for reading out the forces)
     */
...
#        endif
         ...
         level++) /* note: for level=0, target is the same task */
    {
        ...
        if (recvTask < NTask)
        {
            ...
            sendmin = 2 * GRID;
            sendmax = -GRID;
            for (slab_x = meshmin_list[3 * recvTask] - 2;
                 slab_x < meshmax_list[3 * recvTask] + 4; slab_x++)
                if (slab_to_task[slab_x] == sendTask)
                {
                    if (slab_x < sendmin)
                        sendmin = slab_x;
            ...
            {
                recv_dimx = meshmax_list[3 * recvTask + 0] -
                            meshmin_list[3 * recvTask + 0] + 6;
                recv_dimy = meshmax_list[3 * recvTask + 1] -
                            meshmin_list[3 * recvTask + 1] + 6;
                recv_dimz = meshmax_list[3 * recvTask + 2] -
                            meshmin_list[3 * recvTask + 2] + 6;
                ...
                if (level > 0)
                {
                    MPI_Sendrecv(
                        ...
                        (recvmax - recvmin + 1) * dimy * dimz *
                            sizeof(fftw_real),
                        MPI_BYTE, recvTask, TAG_NONPERIOD_B, MPI_COMM_WORLD,
                        &status);
                }
                else
                {
    ...
    for (dim = 0; dim < 3; dim++) /* Calculate each component of the force. */
    {
        ...
        /* note: "workspace" now contains the potential for the local patch,
         * plus a suffiently large buffer region */
...
#        endif
            ...
            acc_dim = forcegrid[(slab_x * dimy + slab_y) * dimz + slab_z] *
                      (1.0 - dx) * (1.0 - dy) * (1.0 - dz);
            acc_dim += forcegrid[(slab_x * dimy + slab_yy) * dimz + slab_z] *
                       (1.0 - dx) * dy * (1.0 - dz);
            acc_dim += forcegrid[(slab_x * dimy + slab_y) * dimz + slab_zz] *
                       (1.0 - dx) * (1.0 - dy) * dz;
            acc_dim += forcegrid[(slab_x * dimy + slab_yy) * dimz + slab_zz] *
                       (1.0 - dx) * dy * dz;

            acc_dim += forcegrid[(slab_xx * dimy + slab_y) * dimz + slab_z] *
                       (dx) * (1.0 - dy) * (1.0 - dz);
    ...
    All.NumForcesSinceLastDomainDecomp =
        1 + All.TotNumPart * All.TreeDomainUpdateFrequency;
                    ...
                    0) /* ok, we have a contribution to the slab */
            {
                ...
                if (level > 0)
                {
                    MPI_Sendrecv(
                        ...
                        (sendmax - sendmin + 1) * dimy * dimz *
                            sizeof(fftw_real),
                        MPI_BYTE, recvTask, TAG_NONPERIOD_C, forcegrid,
                        (recvmax - recvmin + 1) * recv_dimy * recv_dimz *
                            sizeof(fftw_real),
                        MPI_BYTE, recvTask, TAG_NONPERIOD_C, MPI_COMM_WORLD,
                        &status);
                }
                else
                {
...
#        endif
                    ...
                    0) /* ok, we have a contribution to the slab */
            {
                ...
                if (level > 0)
                {
                    MPI_Sendrecv(
                        ...
                        (recvmax - recvmin + 1) * dimy * dimz *
                            sizeof(fftw_real),
                        MPI_BYTE, recvTask, TAG_NONPERIOD_D, MPI_COMM_WORLD,
                        &status);
                }
                else
                {
    ...
    for (x = 0; x < meshmax[0] - meshmin[0] + 2; x++)
        for (y = 0; y < meshmax[1] - meshmin[1] + 2; y++)
            ...
            {
                forcegrid[(x * dimy + y) * dimz + z] =
                    workspace[((x + 2) * recv_dimy + (y + 2)) * recv_dimz +
                              (z + 2)];
            }
...
#        endif
        ...
        pot = forcegrid[(slab_x * dimy + slab_y) * dimz + slab_z] * (1.0 - dx) *
              (1.0 - dy) * (1.0 - dz);
        pot += forcegrid[(slab_x * dimy + slab_yy) * dimz + slab_z] *
               (1.0 - dx) * dy * (1.0 - dz);
        pot += forcegrid[(slab_x * dimy + slab_y) * dimz + slab_zz] *
               (1.0 - dx) * (1.0 - dy) * dz;
        pot += forcegrid[(slab_x * dimy + slab_yy) * dimz + slab_zz] *
               (1.0 - dx) * dy * dz;

        pot += forcegrid[(slab_xx * dimy + slab_y) * dimz + slab_z] * (dx) *
               (1.0 - dy) * (1.0 - dz);
        pot += forcegrid[(slab_xx * dimy + slab_yy) * dimz + slab_z] * (dx)*dy *
               (1.0 - dz);
        pot += forcegrid[(slab_xx * dimy + slab_y) * dimz + slab_zz] * (dx) *
               (1.0 - dy) * dz;
        pot += forcegrid[(slab_xx * dimy + slab_yy) * dimz + slab_zz] *
               (dx)*dy * dz;

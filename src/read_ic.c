...
void read_ic(char* fname)
{
    ...
    while (rest_files > NTask)
    {
        ...
        {
            if (ThisTask ==
                (groupMaster + gr)) /* ok, it's this processor's turn */
    ...
    if (rest_files > 0)
    {
        ...
        for (gr = 0; gr < ngroups; gr++)
        {
            if ((filenr / All.NumFilesWrittenInParallel) ==
                gr) /* ok, it's this processor's turn */
    ...
    /* this makes sure that masses are initialized in the case that the
       mass-block is completely empty */
    for (i = 0; i < NumPart; i++)
    {
    ...
    if (RestartFlag == 0)
    {
        if (All.InitGasTemp > 0)
        {
            ...
            u_init *=
                All.UnitMass_in_g / All.UnitEnergy_in_cgs; /* unit conversion */
            ...
            for (i = 0; i < N_gas; i++)
            {
                ...
                /* Note: the coversion to entropy will be done in the function
                   init(), after the densities have been computed */
            }
        }
    }
...
#define SKIP2                                                                  \
        ...
        if (All.ICFormat == 1 || All.ICFormat == 2)
        {
            ...
            if (All.ICFormat == 2)
            {
                ...
                printf("Reading header => '%c%c%c%c' (%d byte)\n", label[0],
                       label[1], label[2], label[3], nextblock);
                SKIP2;
            }
    ...
    if (All.TotNumPart == 0)
    {
        ...
        All.TotN_gas = header.npartTotal[0] +
        ...
        All.MaxPart =
            All.PartAllocFactor *
            (All.TotNumPart /
             NTask); /* sets the maximum number of particles that may */
        All.MaxPartSph =
            All.PartAllocFactor * (All.TotN_gas / NTask); /* sets the maximum
                         number of particles that may reside on a processor */
        allocate_memory();
    ...
    if (ThisTask == readTask)
    {
        ...
        printf(
            ...
            (int)(header.npartTotal[0] / 1000000000),
            (int)(header.npartTotal[0] % 1000000000), All.MassTable[0],
            header.npart[1], (int)(header.npartTotal[1] / 1000000000),
            (int)(header.npartTotal[1] % 1000000000), All.MassTable[1],
            header.npart[2], (int)(header.npartTotal[2] / 1000000000),
            (int)(header.npartTotal[2] % 1000000000), All.MassTable[2],
            header.npart[3], (int)(header.npartTotal[3] / 1000000000),
            (int)(header.npartTotal[3] % 1000000000), All.MassTable[3],
            header.npart[4], (int)(header.npartTotal[4] / 1000000000),
            (int)(header.npartTotal[4] % 1000000000), All.MassTable[4],
            header.npart[5], (int)(header.npartTotal[5] / 1000000000),
            (int)(header.npartTotal[5] % 1000000000), All.MassTable[5]);
        fflush(stdout);
    }
    ...
    for (blocknr = 0; blocknr < IO_NBLOCKS; blocknr++)
    {
        if (blockpresent(blocknr))
        {
            ...
            blockmaxlen =
                ((int)(All.BufferSize * 1024 * 1024)) / bytes_per_blockelement;
                    ...
                    if (All.ICFormat == 2)
                    {
                        ...
                        if (strncmp(label, Tab_IO_Labels[blocknr], 4) != 0)
                        {
                            ...
                            printf("expected '%c%c%c%c' but found '%c%c%c%c'\n",
                                   label[0], label[1], label[2], label[3],
                                   Tab_IO_Labels[blocknr][0],
                                   Tab_IO_Labels[blocknr][1],
                                   Tab_IO_Labels[blocknr][2],
                                   Tab_IO_Labels[blocknr][3]);
                            fflush(stdout);
                            endrun(1890);
                        }
                    }
                            ...
                            do
                            {
                                ...
                                if (ThisTask == readTask)
                                {
                                    ...
                                    if (All.ICFormat == 3)
                                    {
                                        ...
                                        H5Sselect_hyperslab(
                                            hdf5_dataspace_in_file,
                                            H5S_SELECT_SET, start, NULL, count,
                                            NULL);
                     ...
                     int* last)
{
    ...
    if (nfiles > 1)
    {
        ...
        distribute_file(filesleft, firstfile, firsttask,
                        firsttask + tasksleft - 1, filenr, master, last);
        distribute_file(filesright, firstfile + filesleft,
                        firsttask + tasksleft, lasttask, filenr, master, last);
    }
    else
    {

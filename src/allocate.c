...
void allocate_commbuffers(void)
{
  ...
  --> 8-byte alignment for 64bit processors */
    ...
    GravDataResult =
        GravDataGet; /* this will overwrite the GravDataGet-Table */
    ...
    All.BunchSizeHydro =
        (All.BufferSize * 1024 * 1024) /
        (2 * sizeof(struct hydrodata_in) + 2 * sizeof(struct hydrodata_out));

    HydroDataIn = (struct hydrodata_in*)CommBuffer;
    HydroDataGet = HydroDataIn + All.BunchSizeHydro;
    HydroDataResult =
        (struct hydrodata_out*)(HydroDataGet + All.BunchSizeHydro);
    HydroDataPartialResult = HydroDataResult + All.BunchSizeHydro;

    All.BunchSizeDomain = (All.BufferSize * 1024 * 1024) /
                          (sizeof(struct particle_data) +
                           sizeof(struct sph_particle_data) + sizeof(peanokey));
  ...
  --> 8-byte alignment of DomainKeyBuf for 64bit processors */
    ...
    DomainSphBuf =
        (struct sph_particle_data*)(DomainPartBuf + All.BunchSizeDomain);
    ...
    if (ThisTask == 0)
    {
        printf("\nAllocated %d MByte communication buffer per processor.\n\n",
               All.BufferSize);
        printf(
            "Communication buffer has room for %d particles in gravity "
            "computation\n",
            All.BunchSizeForce);
        printf(
            "Communication buffer has room for %d particles in density "
            "computation\n",
            All.BunchSizeDensity);
        printf(
            "Communication buffer has room for %d particles in hydro "
            "computation\n",
            All.BunchSizeHydro);
        printf(
            "Communication buffer has room for %d particles in domain "
            "decomposition\n",
            All.BunchSizeDomain);
        printf("\n");
    }
...
void allocate_memory(void)
{
    ...
    if (All.MaxPartSph > 0)
    {
        ...
        if (!(SphP = malloc(bytes = All.MaxPartSph *
